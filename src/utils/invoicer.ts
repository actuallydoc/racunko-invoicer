...
export default function generatePDFInvoice(invoice: InvoiceSerialized): URL {
    let doc = new jsPDF;
...
const renderPaymentData = (doc: jsPDF, invoice: InvoiceSerialized, customer: Partner, company: Company) => {
    doc.setFontSize(10);
...
const renderTable = (doc: jsPDF, company: Company, invoice: InvoiceSerialized) => {
    let finalY = 0;
...
const renderOurCompany = (doc: jsPDF, company: Company, invoice: InvoiceSerialized) => {
